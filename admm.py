...
"""
Compressed sensing for images sampled at a subset of rows in k-space.
...
def runtestadmm(method, cpu, filename, mu=1e12, beta=1, subsampling_factor=0.7,
    ...
    subsampling_factor : float
        probability of retaining a row
    n_iter : int
        number of ADMM iterations to conduct
    seed : int
    ...
    mask = np.random.uniform(size=m) < subsampling_factor
    ...
    if method == 'cs_baseline':
        ...
        if cpu:
            x, loss = cs_fft(m, n, f, mask, mu=mu, beta=beta,
                             n_iter=n_iter)
        else:
            # Move the mask and Fourier data to the GPUs.
            ...
            ctorch.fft2(f_th)
            t = tic()
            x, loss = cs_fft(m, n, f_th, mask_th, mu=mu, beta=beta,
                             n_iter=n_iter)
            x = x.cpu().numpy()
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image', default='brain.png')
